...
function initLogin() {
    ...
    form.addEventListener("submit", async (event) => {
        ...
        setText("login-error", getErrorMessage(error, "Login failed"));
    });
}
...
async function submitProductForm(event) {
    ...
    const error = await safeParseJson(response);
    alert(getErrorMessage(error, "Could not save product"));
}
...
function initUsers() {
    document.getElementById("logout").addEventListener("click", logout);
    document.getElementById("user-search-button").addEventListener("click", () => loadUsers(0));
    ...
    bindEnterKey("user-search-query", () => loadUsers(0));
    loadUsers(0);
}
...
async function loadUsers(page = 0) {
    ...
    const response = await fetchWithAuth(`${API_URL}/admin/users?${params.toString()}`);
    if (!response.ok) {
    ...
    renderPagination(data.number, data.totalPages, loadUsers);
    setText("user-search-label", query ? query.toUpperCase() : "ALL");
    setText("users-page-label", `Page ${data.number + 1}`);
}
